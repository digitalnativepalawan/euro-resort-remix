...
const ActionRequiredPanel = () => {
  ...
  const urgencyConfig: Record<UrgencyLevel, { bar: string; label: string; labelClass: string; icon: React.ReactNode }> = {
    overdue: { bar: 'bg-destructive', label: t('actionRequired.overdue'), labelClass: 'text-destructive', icon: <AlertTriangle className="w-3 h-3" /> },
    today: { bar: 'bg-warning', label: t('actionRequired.dueToday'), labelClass: 'text-warning', icon: <Clock className="w-3 h-3" /> },
    inprogress: { bar: 'bg-primary', label: t('actionRequired.inProgress'), labelClass: 'text-primary', icon: <ArrowRight className="w-3 h-3" /> },
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      const [empResult, commentResult] = await Promise.all([
        empIds.length > 0 ? supabase.from('employees').select('id, name, display_name').in('id', empIds) : Promise.resolve({ data: null }),
        taskIds.length > 0 ? (supabase.from('task_comments' as any) as any).select('task_id').in('task_id', taskIds) : Promise.resolve({ data: null }),
      ]);
  ...
  if (tasks.length === 0) {
    return (
      <div className="mb-4 rounded-lg border border-border bg-card px-4 py-3 flex items-center gap-3">
        ...
        <div>
          <p className="font-display text-xs tracking-wider text-foreground">{t('actionRequired.allClear')}</p>
          <p className="font-body text-xs text-muted-foreground">{t('actionRequired.noTasksNeedAttention')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mb-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="font-display text-xs tracking-widest text-muted-foreground uppercase">{t('actionRequired.title')}</h2>
        <button onClick={() => navigate('/employee-portal')} className="font-body text-xs text-primary flex items-center gap-0.5 hover:underline">
      ...
      <div className="space-y-2">
        {tasks.map(task => {
          ...
          const dueDateLabel = task.due_date && urgency === 'pending'
            ? t('actionRequired.due', { date: formatShortDate(task.due_date) })
            : '';
              ...
              <div className="flex flex-1 items-center gap-3 px-3 py-3 min-w-0">
                ...
                {(() => {
                  ...
                  const buttonLabel = isMyTask
                    ? (task.status === 'in_progress' ? t('common.continue') : t('actionRequired.startTask'))
                    : t('common.manage');
                  const buttonVariant = !isMyTask ? 'outline' : urgency === 'overdue' ? 'destructive' : 'default';
                  return (
                    <Button size="sm" variant={buttonVariant} className="shrink-0 font-display text-xs tracking-wide h-8 px-3"
